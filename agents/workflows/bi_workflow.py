...
def run_bi_workflow(user_question: str) -> None:
    ...
    critic_decision = get_normalized_critic_decision(critic_output)

    if "## Table Validator\n\nOK" in table_validation_output:
        if critic_decision == "REQUIERE REVISIÓN" and "NO CONCLUYENTE" in critic_output.upper():
            critic_decision = "APROBADA"

    print("\n[Critic Decision]")
    print(critic_decision)

    if critic_requires_revision(critic_output) and critic_decision == "REQUIERE REVISIÓN":
        print("\n[Revisión automática]")
